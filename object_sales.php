    ...
    <div class="b-objects-slide">
        ...
        <div class="l-objects">
            ...
            <p class="b-objects__content">
                <?php the_field('pages_descripton'); ?>
            </p>
        </div>
    </div>
</div>
...
<div class="bg-wrapper">
    <?php
        $mypost = array( 'post_type' => 'trio_objects', );
        $loop = new WP_Query( $mypost );
        while ( $loop->have_posts() ) : $loop->the_post();
            if( has_term('objects_on_sale', 'trio_objects_category') ) :  ?>
                <div class="l-object-info clearfix">
                    <h3 class="b-object-info__name"><?php the_title($loop->ID); ?></h3>
                    <a href="<?php the_field('sale_object_link', $loop->ID); ?>" class="b-object-info__site" target="_blank"><?php the_field('sale_object_link', $loop->ID); ?></a>
                </div>
                <div class="b-released-item-content clearfix">
                    ...
                    <?php if (get_field('total_area_of_object')) :  ?>
                        <div class="b-releazed-item-content__property-name">Общая площадь объекта, м2:
